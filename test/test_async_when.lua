...
local async_any_state = require "dromozoa.socks.async_any_state"
...
assert(service:dispatch(coroutine.create(function ()
  ...
  async_future(async_any_state(service, f1, f2, f3)):wait()
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(not f3:is_ready())

  async_future(async_any_state(service, f1, f3)):wait()
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())

  async_future(async_any_state(service, f1)):wait_for(0.2)
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())
