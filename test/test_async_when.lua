...
assert(service:dispatch(coroutine.create(function ()
  ...
  future(latch_state("any", f1, f2, f3)):wait()
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(not f3:is_ready())

  future(latch_state("any", f1, f3)):wait()
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())

  assert(future(latch_state("any", f1)):wait_for(0.2) == "timeout")
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())

  assert(future(latch_state("any", f1)):wait_for(0.5) == "ready")
  assert(f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())
  ...
  assert(future(latch_state("all", f1, f2, f3)):wait_for(0.2) == "timeout")
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())

  assert(future(latch_state("all", f1, f2, f3)):wait_for(0.5) == "ready")
  assert(not f1:is_ready())
  assert(f2:is_ready())
  assert(f3:is_ready())
